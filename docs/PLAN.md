...
**Цель:** вручную загрузить MP3 и увидеть на сайте транскрипт + отчёт об оценке.
...
- Простая сессионная админка: `admin` видит всё, `doctor` работает только со своими записями
...
- `admin` / `adm1n` — полный доступ ко всем записям
- `doctor` / `doctor` — видит и удаляет только записи, где `doctor_name = doctor`; при загрузке это поле подставляется автоматически
